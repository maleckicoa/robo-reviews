...
def assign_product_category(review_emb):
    """
    Assign the most similar category to a review embedding
    """
...
def get_product_categories(df, sample_size, cost_per_million_tokens):
    """
    Pass the reviews through OpenAI model top get product categories
    Save the product categories to a pickle file
    """
    product_type_dict = {}
    start_index = 0
    sample_no = 1
    ...
    while start_index < len(df):
        end_index = min(start_index + sample_size, len(df))

        review_list = df["name_title_text"].iloc[start_index:end_index].tolist()
        review_text = "\n".join(review_list)
        ...
        try:
            ...
            product_type_dict[f"Cost: {sample_no}"] = (
                float(usage["total"]) / 1_000_000 * cost_per_million_tokens
            )
            print(f"Categories {sample_no}:  {text}")
            print(f"Usage: {usage}")
            print(
                f"Cost: ${float(usage['total']) / 1_000_000 * cost_per_million_tokens}"
            )
...
def make_embeddings(df_resampled, sample_size, cost_per_million_tokens):
    """
    Pass the reviews through OpenAI model to get embeddings
...
def main():
    ...
    It creates 2 pickle files:
    - data/embedding_columns.pkl
    - data/category_columns.pkl
    """
    ...
    all_categories = [
        v for k, v in product_type_dict.items() if k.startswith("Categories")
    ]
    categores_text = "\n".join(all_categories)
    categores_prompt = f"{prompt_categories_header}\n\n{categores_text}"
    text, usage = get_completion(categores_prompt)
    product_categories = [p.strip() for p in text.split(",") if p.strip()]
    print(product_categories)

    # MAKE REVIEW EMBEDDINGS (for Name, Title, Review)
    embeddings, usage_dict = make_embeddings(
        df_resampled, sample_size=100, cost_per_million_tokens=0.02
    )
    df_resampled["review_embedding"] = embeddings

    # MAKE PRODUCT CATEGORIES EMBEDDINGS
    global product_categories_embeddings
    product_categories_embeddings = {}
    ...
    df_resampled["predicted_product_category"] = df_resampled["review_embedding"].apply(
    ...
    print("\n\nModel 2 completed")
    return category_columns, embedding_columns
